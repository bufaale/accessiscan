...
export function AuditCheckoutForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-xl border-2 border-[#0b1f3a] bg-[#0b1f3a] p-6 text-white">
      <div className="flex items-baseline justify-between">
        <h2 className="text-lg font-semibold">Get your audit</h2>
      ...
      <label className="mt-4 block">
        <span className="text-sm font-medium">Website URL to audit</span>
        <input
          type="text"
          value={url}
      ...
      {error ? (
        <p className="mt-3 text-sm text-amber-200" role="alert" data-testid="audit-error">
          {error}
        </p>
      ) : null}
