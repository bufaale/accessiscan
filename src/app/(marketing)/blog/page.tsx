      ...
      <section className="mx-auto max-w-[1100px] px-6 py-20">
        <div className="grid gap-6 md:grid-cols-2">
          {sorted.map((p) => (
            ...
            >
              <div className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#06b6d4]">
                <span>{CATEGORY_LABEL[p.category] ?? p.category}</span>
                <span className="text-slate-300">·</span>
                <span className="text-slate-500">{p.readMinutes} min read</span>
              </div>
              <h2 className="mt-3 font-display text-xl font-semibold leading-tight text-[#0b1f3a] group-hover:text-[#06b6d4]">
                {p.title}
              </h2>
